...
- **Logical-design work item:** S0-010
- **Enforceable-schema work item:** S1-008
- **Status:** Approved logical design with an approved initial physical scope
- **Purpose:** Maintain the target domain model and define the enforceable schema slice introduced by S1-008.
- **Normative dependencies:** `docs/data-conventions.md`, decision D-09 and the S1-007 controlled state-transition service
- **Test campaign:** `MC-REG-001`
- **Updated:** 2026-07-23

## 1. Purpose

This document maintains the approved logical core schema for Marketing Content and records which portion becomes physically enforceable during Sprint 1.

The complete domain inventory remains the approved target model. S1-008 does not implement that inventory in full: its new physical business tables are limited to `opportunities` and `campaigns`, supported by foundations already established in earlier Sprint 1 work items.

This document does not authorize production data, real leads, unrestricted remote migrations or expansion beyond the approved S1-008 scope.
...
### 3.1 Approved logical target model
...
- Outbox, imports, audit and controlled state transitions.

This inventory describes the approved logical destination. It does not authorize every listed entity to be created by S1-008.

### 3.2 S1-008 physical implementation scope

S1-008 introduces the first enforceable business-schema slice:

- `opportunities`;
- `campaigns`;
- UUID primary keys;
- database-generated immutable human codes;
- required ownership and relationship foreign keys;
- UTC-compatible lifecycle timestamps;
- actor attribution and optimistic-concurrency fields;
- integration of opportunity and campaign lifecycle state with the relational service established by S1-007.

The current lifecycle state is maintained by `state_transition_subjects.current_state`. It must not be duplicated as an enum-backed or vocabulary-constrained state column in `opportunities` or `campaigns`.

### 3.3 Deferred from S1-008

The following remain outside the new physical schema introduced by S1-008:

- `leads` and related restricted-data tables, until the pending schema-separation decision is resolved;
- the remaining logical domain tables, until their corresponding work items authorize them;
- real leads or other production PII;
- production deployment or production data loading.

Existing shared foundations created by S1-001 through S1-007 remain dependencies and are not recreated by S1-008.

### 3.4 Product exclusions

The following remain outside Marketing Content scope:
...
`id` is the UUID primary key. `code` is generated by the database and is immutable after creation. The current lifecycle state is maintained exclusively through `state_transition_subjects.current_state`; `opportunities` does not duplicate it in a `status` column.
### 10.4 `sources`
...
`id` is the UUID primary key. `code` is generated by the database and is immutable after creation. The current lifecycle state is maintained exclusively through `state_transition_subjects.current_state`; `campaigns` does not duplicate it in a `status` column.
