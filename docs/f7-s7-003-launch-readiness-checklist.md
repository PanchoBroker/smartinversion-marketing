...
| Date | 2026-08-10 (row 4 updated same day, §8); rows 1, 2 and 3 updated 2026-08-11, §8 |
...
|---|---|---|---|---|
| 1 | D-06 — Consent and privacy | `docs/decision-register.md` §8 | **Decided (2026-08-11).** The product owner approved the notice text directly, without prior external legal review, per D-19 (§21). Text includes the income-field sensitive-data disclosure (Art. 16), the 6-month retention statement (D-07), the public campaign name "Campaña v1," and the ARCO+ contact `contacto@smartinversion.cl`. Draft identifiers (`contact_data_v1_draft`) remain synthetic-only until wired to a real form — an implementation step, not a condition of this row. | Closed. Still open, but not this row: D-08 and G0-R05 independently block real launch; a correction/withdrawal mechanism and staffing the ARCO+ inbox remain deferred (`docs/decision-register.md` §8 Residual condition). |
...
**F7 real launch is NOT ready.** Rows 1 and 2 (D-06, D-07) both closed 2026-08-11 — D-06 via direct product-owner approval under D-19 (no external legal review required), D-07 for the non-converting-lead case. Two conditions remain genuinely open: row 3 (D-08) was formally re-reviewed 2026-08-11 and confirmed still undefined (the campaign itself has not been scoped), and row 4 (MFA/G0-R05) is only partially resolved. One scope question (row 5) has not even been formally raised yet. Row 4 improved 2026-08-10 (§8): its MFA sub-item is now closed with real evidence, but the row as a whole is still open (named real-human role assignment, hosted-project session verification) — closing one of three sub-items does not close the row. Rows 6-7 are not blockers today, but row 6 exists only because the launch itself hasn't started — it converts to a live risk the moment rows 3-4 clear and a real integration begins.
...
- ~~Product owner: finalize D-06 consent wording.~~ Done 2026-08-11 — approved directly by the product owner, no external legal review required (D-19). Still open and deferred: correction/withdrawal mechanism, and staffing the ARCO+ inbox (`docs/decision-register.md` §8 Residual condition).
...
- **2026-08-11 (same day, second update).** Row 2 (D-07) closed for non-converting leads: after a full read of the Ley 21.719 official text (BCN PDF, 56 pages), the product owner decided directly on a 6-month retention period from the last interaction event, grounded in Art. 3° letra c). `docs/decision-register.md` §9 updated from `Conditioned` to `Decided (non-converting leads only)`. Retention for converting leads is explicitly not covered and remains open. Also recorded the same day: D-19 (`docs/decision-register.md` §21), the product owner's informed decision to launch without prior external legal review, accepting the risk directly.
- **2026-08-11 (same day, third update).** Row 1 (D-06) closed: the product owner approved the notice text directly, exercising the D-19 waiver above, resolving its two remaining placeholders — public campaign name ("Campaña v1," a generic versionable label decoupled from D-08's still-open scope) and ARCO+ contact address (`contacto@smartinversion.cl`). `docs/decision-register.md` §8 updated from `Conditioned` to `Decided`. §4 updated to reflect that rows 1-2 are now both closed; rows 3-4 remain the real blockers to launch.
