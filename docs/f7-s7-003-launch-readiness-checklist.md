...
| Date | 2026-08-10 (row 4 updated same day, §8); row 3 updated 2026-08-11, §8 |
...
| 2 | D-07 — Lead retention | `docs/decision-register.md` §9 | **Conditioned.** Direction fixed (configurable, purpose-bound, verifiable expiration/anonymization/deletion); no final retention period approved. No duration invented here — `docs/d06-d07-consent-retention-draft-proposal.md` §3 offers reference ranges only, deliberately no proposed number, per Gate G0's own "no retention period is inferred or invented" condition. | Legal/operational approval of a specific retention period, entered as a decision-register update. |
| 3 | D-08 — `MC-REG-001` pilot scope | `docs/decision-register.md` §10; finding restated in `docs/f7-pilot-contract.md` §10 | **Conditioned (reconfirmed 2026-08-11).** D-08's original provisional decision had lapsed — its own text said it "expires before Phase 3 begins," and Phase 3 closed (per Gate G3/G4 records) without a formal revisit. That governance gap is now closed: the product owner confirmed directly that `MC-REG-001` simply has not been designed yet, and status moved from `Provisional` to `Conditioned`. Eight open scope elements (cities/regions, projects, thesis, platforms, organic/paid mix, budget cap, owners, start/pause/stop criteria) remain genuinely unapproved — reconfirming the decision's status did not fill in any of them. | Product owner designs `MC-REG-001` and approves all eight scope elements. |
...
- Product owner: design `MC-REG-001` and approve its eight scope elements (the decision's status itself is already reconfirmed as `Conditioned`, 2026-08-11 — see §8 below).
...
- **2026-08-11.** Row 3 (D-08) updated: `docs/decision-register.md` §10 status moved from `Provisional` (lapsed, unreviewed) to `Conditioned` (reviewed; product owner confirmed the campaign has not been designed yet — not an oversight). This closes the governance gap this checklist itself first flagged; none of the eight open scope elements were filled in. `docs/f7-pilot-contract.md` §10 updated in the same change.
