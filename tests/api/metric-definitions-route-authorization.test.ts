...
import { POST as createMetricDefinition } from "@/app/api/v1/metric-definitions/route";

// S5-008 (iteration 2/N): third F5 private route, same plain userClient +
// RLS shape as publications-route-authorization.test.ts /
// tracking-links-route-authorization.test.ts (S5-008 iteration 1).

const CORRELATION_ID = "a73e4567-e89b-42d3-a456-426614174014";
...
function fakeUserClient(user: { id: string } | null = { id: "auth-user" }) {
  const single = vi.fn(async () => ({
    data: { id: "90000000-0000-4000-8000-000000000301" },
    error: null,
  }));
...
const VALID_METRIC_DEFINITION = {
  name: "ctr",
  unit: "percentage",
  formula: "clicks / impressions",
};

function metricDefinitionRequest(body: Record<string, unknown>) {
  return new Request("http://localhost/api/v1/metric-definitions", {
    method: "POST",
    headers: {
...
describe("metric-definitions route authorization (plain userClient + RLS)", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("denies an unauthenticated request before touching any data", async () => {
    ...
    const response = await createMetricDefinition(
      metricDefinitionRequest(VALID_METRIC_DEFINITION),
    );
  ...
  it("denies a role the policy does not permit, never touching the database", async () => {
    ...
    const request = metricDefinitionRequest(VALID_METRIC_DEFINITION);
    request.headers.set("x-exercised-role", "campaign_manager");

    const response = await createMetricDefinition(request);
    ...
    expect(mocks.logWarn).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        context: {
          action: "metric_definition.write",
  ...
  it("lets a results analyst create a metric definition and logs the allowed decision", async () => {
    const userClient = fakeUserClient();
    const serviceClient = fakeServiceClient({
      profile: { id: PROFILE_ID, account_status: "active" },
  ...
  it("rejects a caller-supplied status at the boundary without inserting", async () => {
    const userClient = fakeUserClient();
    const serviceClient = fakeServiceClient({
      profile: { id: PROFILE_ID, account_status: "active" },
      assignments: [assignment("results_analyst")],
  ...
  it("rejects a missing required field before any insert", async () => {
    ...
    const response = await createMetricDefinition(
      metricDefinitionRequest(withoutUnit),
    );
