...
import { POST as rejectQa } from "@/app/api/v1/content-versions/[id]/reject-qa/route";

// S4-009: behavioral Private API coverage for the QA rejection gate
// command endpoint, mirroring tests/api/content-version-submit-qa-
// authorization.test.ts's harness. This RPC (public.
// reject_content_version_qa) does not go through the S1-007
// execute_state_transition engine and exposes no expected_version. Unlike
// submit-qa (creative_owner, content_version.write), this route is gated
// by the new content_version.approve action (approver-only).

const CORRELATION_ID = "223e4567-e89b-42d3-a456-426614174010";
...
function rejectQaRequest(body: Record<string, unknown> = {
  reason: "S4-009 fixture QA rejection",
}) {
  return new Request(
    `http://localhost/api/v1/content-versions/${CONTENT_VERSION_ID}/reject-qa`,
    {
      method: "POST",
      headers: {
...
describe("content-version reject-qa authorization (S1-003 then public.reject_content_version_qa)", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("denies an unauthenticated request before touching the RPC", async () => {
    ...
    const response = await rejectQa(rejectQaRequest(), routeContext());

    expect(response.status).toBe(401);
    expect(mocks.createServiceClient).not.toHaveBeenCalled();
  });

  it("denies a role the S1-003 policy does not permit (creative_owner cannot approve/reject QA), never reaching the RPC", async () => {
    ...
    const request = rejectQaRequest();
    request.headers.set("x-exercised-role", "creative_owner");

    const response = await rejectQa(request, routeContext());
  ...
  it("rejects a malformed id before calling the RPC", async () => {
    ...
    const response = await rejectQa(rejectQaRequest(), {
  ...
  it("lets an approver reject a version's QA, calling the RPC with the correlation id and logging the outcome", async () => {
    mocks.createUserClient.mockResolvedValue(
      fakeUserClient({ id: "auth-user" }),
    );
    ...
    expect(mocks.logInfo).toHaveBeenCalledWith(
      expect.objectContaining({
        event: "api.content_version.qa_rejected",
  ...
  it("maps the RPC's own role guard (42501) to 403", async () => {
    ...
    const serviceClient = fakeServiceClient({
      ...
      rpcResult: {
        ...
        error: {
          code: "42501",
          message: "S4_009_ACTIVE_APPROVER_ROLE_REQUIRED",
        },
      },
    });
    mocks.createServiceClient.mockResolvedValue(serviceClient.client);
  ...
  it("maps the not-qa-pending status guard (23514) to 400", async () => {
    ...
    const serviceClient = fakeServiceClient({
      ...
      rpcResult: {
        ...
        error: {
          code: "23514",
          message: "S4_009_CONTENT_VERSION_NOT_QA_PENDING",
        },
      },
    });
    mocks.createServiceClient.mockResolvedValue(serviceClient.client);
