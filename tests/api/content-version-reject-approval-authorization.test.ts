...
import { POST as rejectApproval } from "@/app/api/v1/content-versions/[id]/reject-approval/route";

// S4-009: behavioral Private API coverage for the approval rejection
// command endpoint, mirroring tests/api/content-version-reject-qa-
// authorization.test.ts's harness. This RPC (public.
// reject_content_version_approval) does not go through the S1-007
// execute_state_transition engine and exposes no expected_version. Same
// content_version.approve (approver-only) gate as reject-qa/promote/
// approve, confirmed by direct inspection of the RPC body.

const CORRELATION_ID = "223e4567-e89b-42d3-a456-426614174011";
const PROFILE_ID = "10000000-0000-4000-8000-000000000005";
const CONTENT_VERSION_ID = "30000000-0000-4000-8000-000000000011";
...
function rejectApprovalRequest(body: Record<string, unknown> = {
  reason: "S4-009 fixture approval rejection",
}) {
  return new Request(
    `http://localhost/api/v1/content-versions/${CONTENT_VERSION_ID}/reject-approval`,
    {
      method: "POST",
      headers: {
...
describe("content-version reject-approval authorization (S1-003 then public.reject_content_version_approval)", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("denies an unauthenticated request before touching the RPC", async () => {
    ...
    const response = await rejectApproval(rejectApprovalRequest(), routeContext());

    expect(response.status).toBe(401);
    expect(mocks.createServiceClient).not.toHaveBeenCalled();
  });

  it("denies a role the S1-003 policy does not permit (creative_owner cannot approve/reject an approval), never reaching the RPC", async () => {
    ...
    const response = await rejectApproval(request, routeContext());
  ...
  it("rejects a malformed id before calling the RPC", async () => {
    ...
    const response = await rejectApproval(rejectApprovalRequest(), {
  ...
  it("lets an approver reject a version's approval, calling the RPC with the correlation id and logging the outcome", async () => {
    mocks.createUserClient.mockResolvedValue(
      fakeUserClient({ id: "auth-user" }),
    );
    ...
    expect(serviceClient.rpc).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        p_reason: "S4-009 fixture approval rejection",
  ...
  it("maps the not-approval-pending status guard (23514) to 400", async () => {
    ...
    const serviceClient = fakeServiceClient({
      ...
      rpcResult: {
        ...
        error: {
          code: "23514",
          message: "S4_006_CONTENT_VERSION_NOT_APPROVAL_PENDING",
        },
      },
    });
    mocks.createServiceClient.mockResolvedValue(serviceClient.client);
