...
import { POST as createTrackingLink } from "@/app/api/v1/tracking-links/route";

// S5-008 (iteration 1/N): second F5 private route, same plain userClient +
// RLS shape as publications-route-authorization.test.ts.

const CORRELATION_ID = "a63e4567-e89b-42d3-a456-426614174013";
const PROFILE_ID = "10000000-0000-4000-8000-000000000010";
const CAMPAIGN_ID = "70000000-0000-4000-8000-000000000030";
const PUBLICATION_ID = "70000000-0000-4000-8000-000000000031";
...
function fakeUserClient(user: { id: string } | null = { id: "auth-user" }) {
  const single = vi.fn(async () => ({
    data: { id: "90000000-0000-4000-8000-000000000201" },
    error: null,
  }));
...
const VALID_TRACKING_LINK = {
  campaign_id: CAMPAIGN_ID,
  publication_id: PUBLICATION_ID,
  variant: "organic_share",
};

function trackingLinkRequest(body: Record<string, unknown>) {
  return new Request("http://localhost/api/v1/tracking-links", {
    method: "POST",
    headers: {
...
describe("tracking-links route authorization (plain userClient + RLS)", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("denies an unauthenticated request before touching any data", async () => {
    ...
    const response = await createTrackingLink(
      trackingLinkRequest(VALID_TRACKING_LINK),
    );
  ...
  it("denies a role the policy does not permit, never touching the database", async () => {
    ...
    const request = trackingLinkRequest(VALID_TRACKING_LINK);
    request.headers.set("x-exercised-role", "results_analyst");

    const response = await createTrackingLink(request);
  ...
  it("lets a publisher create a tracking link and logs the allowed decision", async () => {
    const userClient = fakeUserClient();
    const serviceClient = fakeServiceClient({
      profile: { id: PROFILE_ID, account_status: "active" },
    ...
    expect(mocks.logInfo).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        context: {
          action: "tracking_link.write",
          exercised_role: "publisher",
        },
      }),
    );
  });

  it("rejects a caller-supplied token at the boundary without inserting", async () => {
    const userClient = fakeUserClient();
    const serviceClient = fakeServiceClient({
      profile: { id: PROFILE_ID, account_status: "active" },
      assignments: [assignment("publisher")],
    ...
    expect(body.details.field).toBe("token");
  ...
  it("rejects a missing required field before any insert", async () => {
    ...
    const withoutVariant: Record<string, unknown> = {
      ...VALID_TRACKING_LINK,
    };
    delete withoutVariant.variant;

    const response = await createTrackingLink(
      trackingLinkRequest(withoutVariant),
    );
